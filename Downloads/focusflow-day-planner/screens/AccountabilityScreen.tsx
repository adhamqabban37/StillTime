...
const AccountabilityScreen: React.FC = () => {
  ...
  const activeHabits = state.goals
    .filter((h) => !h.archived)
    .map((h) => enrichHabitWithStats(h, state.goalLogs));
  ...
  const handleToggle = (habitId: string, status: HabitLogStatus) => {
    ...
    if (status === "done") {
      dispatch({
        type: "SHOW_TOAST",
        payload: { message: "Goal completed! 🎉" },
      });
      ...
      if (willAllBeDone && todaysHabits.length > 1) {
        ...
        dispatch({
          type: "SHOW_TOAST",
          payload: { message: "All goals done for today! 🏆" },
        });
      }
    }
  };
            ...
            {activeHabits.length === 0 ? (
              ...
              <>
                ...
                <p className="text-slate-500 dark:text-slate-400">
                  No goals scheduled for today. Enjoy your break!
                </p>
              </>
            )}
          </div>
