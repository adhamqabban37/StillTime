...
const AddGoalForm: React.FC<AddGoalFormProps> = ({
  ...
  const [frequencyType, setFrequencyType] = useState<HabitFrequencyType>(
    editingGoal?.frequencyType || "daily"
  );
  const [targetPerWeek, setTargetPerWeek] = useState(
    editingGoal?.targetPerWeek || 5
  );
  const [daysOfWeek, setDaysOfWeek] = useState<number[]>(
    editingGoal?.daysOfWeek || [1, 2, 3, 4, 5] // Mon-Fri default
  );
  const [reminderTime, setReminderTime] = useState(
    editingGoal?.reminderTime || ""
  );
  const [streakFreezeEnabled, setStreakFreezeEnabled] = useState(
    editingGoal?.streakFreezeEnabled || false
  );
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (editingGoal) {
      dispatch({ type: "UPDATE_GOAL", payload: habitData });
      dispatch({
        type: "SHOW_TOAST",
        payload: { message: "Goal updated! ✓" },
      });
    } else {
      dispatch({ type: "ADD_GOAL", payload: habitData });
      dispatch({
        type: "SHOW_TOAST",
        payload: { message: "Goal created! ✓" },
      });
    }
    ...
    >
      ...
      >
        ...
        <div className="p-4 border-t border-slate-700/50 flex gap-3">
          ...
          >
            {editingGoal ? "Save Changes" : "Create Habit"}
          </button>
        </div>
      </div>
    </div>
